...
func renderBootstrapBriefing(personas []mcp.Persona, skills []mcp.Skill, domain string) string {
	...
	sb.WriteString("  Omitting the argument falls back to a keyword router. It ranks routes by how\n")
	sb.WriteString("  many keywords match and only matches at word starts, so it is no longer easily\n")
	sb.WriteString("  fooled — but it still cannot read an exclusion: 'not a security question'\n")
	sb.WriteString("  matches the security route and seats those personas anyway. It also knows\n")
	sb.WriteString("  nothing about which voices this particular decision needs. You do.\n")
