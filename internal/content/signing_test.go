...
func TestSignAndVerify(t *testing.T) {
	...
	if !ok {
		t.Errorf("valid signature should verify: %s", status)
	}
}
...
func TestVerifyFailsOnModifiedBody(t *testing.T) {
	...
	if status == "" {
		t.Error("should return status message")
	}
}
...
func TestVerifyFailsOnModifiedTitle(t *testing.T) {
	...
	if ok {
		t.Error("modified title should fail verification")
	}
}
...
func TestVerifyUnsigned(t *testing.T) {
	...
	ok, status := VerifyPiece(p, kp.PublicKeyHex())
	if ok {
		t.Error("unsigned piece should not verify")
	}
	if status == "" {
		t.Error("should return status")
	}
}
...
func TestKeyPairRoundTrip(t *testing.T) {
	...
	if !ok {
		t.Error("should verify with loaded key")
	}
}
...
func TestInvalidPrivateKey(t *testing.T) {
	...
	if err == nil {
		t.Error("wrong length key should return error")
	}
}
