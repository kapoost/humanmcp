...
func TestMessageAllowsLinks(t *testing.T) {
	...
	for _, text := range cases {
		...
		if err != nil {
			t.Errorf("links should be allowed: %q — %v", text, err)
		}
	}
}
...
func TestMessageRejectsHTML(t *testing.T) {
	...
	for _, text := range cases {
		_, err := ms.Save("", text, "")
		if err == nil {
			t.Errorf("should reject HTML: %q", text)
		}
	}
}
...
func TestMessageTruncates(t *testing.T) {
	...
	if len([]rune(m.Text)) > 2000 {
		t.Errorf("text should be truncated to 280, got %d", len(m.Text))
	}
}
...
func TestMessageWithRegarding(t *testing.T) {
	...
	if m.Regarding != "my-slug" {
		t.Errorf("regarding: got %q", m.Regarding)
	}
}
