...
func TestBlobAllTypesRoundtrip(t *testing.T) {
	...
	for _, tc := range types {
		...
		if err != nil {
			t.Errorf("Get %s: %v", tc.blobType, err)
			continue
		}
		if loaded.BlobType != tc.blobType {
			t.Errorf("%s: type mismatch", tc.blobType)
		}
		if loaded.MimeType != tc.mime {
			t.Errorf("%s: mime mismatch", tc.blobType)
		}
		if loaded.TextData != tc.data {
			t.Errorf("%s: data mismatch", tc.blobType)
		}
	}
}
...
func TestBlobFileStorageAndRetrieval(t *testing.T) {
	...
	if len(read) != len(jpegData) {
		t.Errorf("size mismatch: %d != %d", len(read), len(jpegData))
	}
	if read[0] != 0xFF || read[1] != 0xD8 {
		t.Error("JPEG magic bytes corrupted")
	}
}
...
func TestMessageLengthLimit(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("message at limit should save: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("over-limit message should be truncated not error: %v", err)
	}
...
func TestLicenseRoundtripAllTypes(t *testing.T) {
	...
	for _, lic := range licenses {
		...
		if err != nil {
			t.Fatalf("Get(%s): %v", lic, err)
		}
		if loaded.License != lic {
			t.Errorf("license %s: got %q", lic, loaded.License)
		}
...
func TestPriceSatsOnlyForCommercial(t *testing.T) {
	...
	if loaded.License != "free" {
		t.Errorf("license: %q", loaded.License)
	}
}
...
func TestSignatureSetOnSave(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("SignPiece: %v", err)
	}
	p.Signature = sig

	ok, status := VerifyPiece(p, kp.PublicKeyHex())
	if !ok {
		t.Errorf("fresh signature should verify: %s", status)
	}
}
...
func TestSignatureInvalidatedAfterBodyEdit(t *testing.T) {
	...
	if ok {
		t.Error("edited body should invalidate signature")
	}
}
...
func TestSignatureInvalidatedAfterTitleEdit(t *testing.T) {
	...
	ok, _ := VerifyPiece(p, kp.PublicKeyHex())
	if ok {
		t.Error("title change should invalidate signature")
	}
}
...
func TestSignatureInvalidatedAfterSlugEdit(t *testing.T) {
	...
	ok, _ := VerifyPiece(p, kp.PublicKeyHex())
	if ok {
		t.Error("slug change should invalidate signature")
	}
}
...
func TestResignAfterEdit(t *testing.T) {
	...
	ok, _ := VerifyPiece(p, kp.PublicKeyHex())
	if !ok {
		t.Error("re-signed piece should verify")
	}
	if sig1 == sig2 {
		t.Error("new signature should differ from old")
	}
}
...
func TestBlobSignatureRoundtrip(t *testing.T) {
	...
	if !ok {
		t.Errorf("blob should verify: %s", status)
	}
}
...
func TestBlobSignatureInvalidatedOnDataChange(t *testing.T) {
	...
	if ok {
		t.Error("tampered blob data should fail")
	}
}
...
func TestCertificateChain(t *testing.T) {
	...
	if cert.Author != "kapoost" {
		t.Errorf("author: %q", cert.Author)
	}
	if cert.License != "cc-by" {
		t.Errorf("license: %q", cert.License)
	}
	if cert.Signature != sig {
		t.Errorf("signature mismatch")
	}
	if cert.ContentHash == "" {
		t.Error("hash empty")
	}
	if cert.Originality.Combined <= 0 {
	...
	if !ok {
		t.Error("certificate public key should verify the signature")
	}
...
func TestBlobSlugCollisionOverwrites(t *testing.T) {
	...
	if count != 1 {
		t.Errorf("should have 1 entry with that slug, got %d", count)
	}
}
...
func TestBlobFullRoundtrip(t *testing.T) {
	...
	if loaded.BlobType != BlobContact {
		t.Errorf("type: %q", loaded.BlobType)
	}
	if loaded.MimeType != "application/json" {
		t.Errorf("mime: %q", loaded.MimeType)
	}
	if len(loaded.Audience) != 2 {
		t.Errorf("audience: %d entries", len(loaded.Audience))
	}
	if loaded.TextData != original.TextData {
	...
	if !ok {
		t.Errorf("blob signature should verify after roundtrip: %s", status)
	}

	// Access control still works
	if !loaded.IsAccessibleTo("agent", "claude") {
		t.Error("claude should have access")
	}
	if !loaded.IsAccessibleTo("human", "alice") {
		t.Error("alice should have access")
	}
	if loaded.IsAccessibleTo("agent", "gpt") {
