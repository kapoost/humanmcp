...
| 2026-08-12 | `5e90a03+wip`  | `mysloodsiewnia_write`   | yes           | yes         | Wave 2: usunięty `gate(src)` w `registerMysloodsiewniaWrite` (v2 mysloodsiewnia.go — v1 nieadekwatny bo storyboard runner używa v2 mount). Storyboard `write_offline_owner_and_validation` — asercja "owner + valid args + no heartbeat → offline" zwróciła `{"status":"internal_error","error":"bridge queue not configured"}` zamiast `status:offline`. Wave 2 gate przypięty. Pierwsza próba (sabotaż w v1) NIE failowała — dowód że storyboard runner testuje v2, nie v1 (wartościowa obserwacja dla przyszłych sabotage runs — zawsze sprawdzaj który mount jest testowany). |
